...
| Execution Identity Foundations | 98 tests | 20/20 detected, 0 survivors | N/A — pure contracts, no I/O | PENDING AUDIT (Hito 3.81, not yet accepted) |
...
## 15. Execution Identity Foundations (Hito 3.81 — implemented, pending independent adversarial audit)

Three identity value objects plus two account-scoped wrappers, added to close the two decisions ADR-008 escalated. Not yet accepted; do not treat as a stable public contract until an independent audit closes it (see `docs/progress.md`). Nothing here flows through the write-side or read-side yet — `ExecutionRequest`, `ExpectedOpenOrder` and `ExecutionOpenOrder` all keep `order_id: str` unchanged.
