...
- **Fase activa:** Fase 3 — Execution Gateway (Hito 3.60 completado)
...
- 3.60 — Composition root del timeout HTTP desde valor explícito (`http_timeout_factory.py`), 94 tests — `create_http_timeout_seconds(*, timeout_seconds: int | float) -> int | float`; tipos aceptados: `int` y `float` (no bool); `bool` → `TypeError("timeout_seconds must be int or float, got: bool")`; no-int/no-float → `TypeError("timeout_seconds must be int or float, got: {type}")`; `<= 0` → `ValueError("timeout_seconds must be > 0, got: {v}")`; NaN (float): aceptado (nan ≤ 0 evalúa False en Python, pasa los checks); +inf: aceptado; -inf: ValueError; Decimal/Fraction: TypeError; subclasses de `int` y `float` (no bool): aceptadas; sin límite superior; sin default; no convierte (int preservado como int, float como float); se inyecta en `HttpRequestExecutor._timeout_seconds`; no ejecuta HTTP durante composición; no lee variables de entorno; pendiente: composition root desde configuración externa; no se validó conexión real con Bybit Demo; cancelación, consultas y posiciones pendientes — `create_bybit_recv_window_ms(*, recv_window_ms: int) -> int`; tipo productivo: `int` (no existe value object); validación idéntica a `StandardBybitAuthenticator` y `create_bybit_authenticator`: `bool` → `TypeError("recv_window_ms must be int, got: bool")`; no-int → `TypeError("recv_window_ms must be int, got: {type}")`; `<= 0` → `ValueError("recv_window_ms must be > 0, got: {value}")`; int > 0 → aceptado y retornado sin transformación; subclasses de `int` (no bool): aceptadas; sin límite superior; sin default; no convierte, no redondea, no clampea; no lee variables de entorno; no define default; valor inyectado en `StandardBybitAuthenticator._recv_window_ms`; pendiente: composition root desde configuración externa; no se validó conexión real con Bybit Demo; cancelación, consultas y posiciones pendientes — `create_millisecond_clock() -> SystemMillisecondClock`; sin parámetros; implementación concreta: `SystemMillisecondClock`; satisface `MillisecondClock` Protocol; fuente temporal: `time.time_ns()` (nanosegundos Unix epoch, stdlib); unidad: milisegundos; tipo de retorno: `int`; truncamiento: `// 1_000_000` (descarta fracciones de ms, no redondea); representa Unix epoch en ms; no consulta el reloj durante construcción; no recibe secretos ni credenciales; no lee variables de entorno; no implementa sincronización con Bybit; conservado por identidad en `StandardBybitAuthenticator._clock`; cada llamada crea instancia nueva; integrado hasta gateway completo; pendiente: composition root desde configuración externa; no se validó conexión real con Bybit Demo; cancelación, consultas y posiciones pendientes — `create_message_signer() -> HmacSha256Signer`; sin parámetros: `HmacSha256Signer` no tiene `__init__`; implementación concreta: `HmacSha256Signer`; satisface `MessageSigner` Protocol (`@runtime_checkable`); algoritmo: HMAC-SHA256; biblioteca: `hmac` + `hashlib` (stdlib, sin dependencias externas); encoding: UTF-8 para secret y message; digest: SHA-256; formato de salida: hexadecimal lowercase, 64 caracteres; acepta secret vacío y message vacío; no firma durante construcción; no recibe secretos; no lee variables de entorno; conservado por identidad dentro de `StandardBybitAuthenticator`; cada llamada crea instancia nueva (sin singleton); integrado con `create_bybit_authenticator` y con stack completo hasta gateway; pendiente: composition root desde configuración externa; no se validó conexión real con Bybit Demo; cancelación, consultas y posiciones pendientes — `create_bybit_demo_credentials(*, api_key: str, api_secret: str) -> BybitDemoCredentials`; delega directamente al constructor del dataclass `frozen=True`; `api_key`: str no vacío ni whitespace-only → `ValueError`; tipo no-str → `TypeError` con mensaje `"api_key must be str, got: ..."`, bool rechazado como no-str; `api_secret`: misma validación con mensaje `"api_secret must not be empty or whitespace-only"`; `repr` actual: oculta secreto (`api_secret = field(repr=False)`) — solo muestra `api_key`; factory no lee variables de entorno; no registra ni imprime secretos; no valida credenciales contra Bybit; no crea authenticator, signer ni clock; no ejecuta red; cada llamada crea instancia nueva (sin singleton ni caché); valores no transformados (no strip, no normalización); integrada con `create_bybit_authenticator` y con stack completo hasta gateway; pendiente: composition root desde configuración externa (Railway vars / `.env`); no se validó conexión real con Bybit Demo; cancelación, consultas y posiciones pendientes

**Tests totales:** 3068 passing
