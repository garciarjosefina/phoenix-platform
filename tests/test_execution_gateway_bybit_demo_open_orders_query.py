...
import execution_gateway.bybit_demo_open_orders_query as _module
...
def _order_item(**overrides):
    defaults = dict(
        orderId="bybit-order-1", orderLinkId="phoenix-order-1", symbol="BTCUSDT",
        side="Buy", orderType="Limit", qty="0.01", price="60000.5",
        cumExecQty="0", orderStatus="New", reduceOnly=False,
    )
...
class TestImport:
    def test_importable_directly(self):
        from execution_gateway.bybit_demo_open_orders_query import (
            query_bybit_demo_open_orders as f,
        )
        assert f is query_bybit_demo_open_orders

    def test_importable_from_package(self):
        assert hasattr(execution_gateway, "query_bybit_demo_open_orders")
        assert execution_gateway.query_bybit_demo_open_orders is query_bybit_demo_open_orders

    def test_in_all(self):
        assert "query_bybit_demo_open_orders" in execution_gateway.__all__

    def test_callable(self):
        assert callable(query_bybit_demo_open_orders)


class TestSignature:
    def test_exactly_one_parameter(self):
        sig = inspect.signature(query_bybit_demo_open_orders)
    ...
    def test_return_annotation_is_open_orders_snapshot(self):
        hints = inspect.get_annotations(query_bybit_demo_open_orders, eval_str=True)
        assert hints.get("return") is OpenOrdersSnapshot

    def test_no_positional_args_accepted(self):
        with pytest.raises(TypeError):
            query_bybit_demo_open_orders(_VALID_ENV)
...
class TestSuccess:
    def test_returns_open_orders_snapshot(self, monkeypatch):
        _install_fake_urlopen(monkeypatch)
        result = query_bybit_demo_open_orders(environ=_VALID_ENV)
        assert isinstance(result, OpenOrdersSnapshot)

    def test_empty_orders_list_is_success(self, monkeypatch):
        _install_fake_urlopen(monkeypatch, body=_ok_body(items=[]))
        result = query_bybit_demo_open_orders(environ=_VALID_ENV)
        assert result.orders == ()

    def test_single_order_mapped(self, monkeypatch):
        _install_fake_urlopen(monkeypatch, body=_ok_body(items=[_order_item(symbol="ETHUSDT")]))
        result = query_bybit_demo_open_orders(environ=_VALID_ENV)
        assert len(result.orders) == 1
        assert result.orders[0].symbol == "ETHUSDT"
    ...
    def test_quantity_is_decimal_not_float(self, monkeypatch):
        _install_fake_urlopen(monkeypatch, body=_ok_body(items=[_order_item(qty="0.015")]))
        result = query_bybit_demo_open_orders(environ=_VALID_ENV)
        assert result.orders[0].quantity == Decimal("0.015")

    def test_identity_preserved_end_to_end(self, monkeypatch):
        _install_fake_urlopen(monkeypatch, body=_ok_body(
            items=[_order_item(orderId="bybit-99", orderLinkId="phoenix-42")]
        ))
        result = query_bybit_demo_open_orders(environ=_VALID_ENV)
        assert result.orders[0].exchange_order_id == "bybit-99"
        assert result.orders[0].order_id == "phoenix-42"

    def test_orphan_order_preserved_end_to_end(self, monkeypatch):
        _install_fake_urlopen(monkeypatch, body=_ok_body(items=[_order_item(orderLinkId="")]))
        result = query_bybit_demo_open_orders(environ=_VALID_ENV)
        assert len(result.orders) == 1
        assert result.orders[0].order_id is None
...
class TestInvalidCredentialsAndEnvironment:
    def test_missing_api_key(self, monkeypatch):
        ...
        with pytest.raises(EnvironmentConfigurationError, match="PHOENIX_BYBIT_DEMO_API_KEY"):
            query_bybit_demo_open_orders(environ=env)
        assert called == []
    ...
    def test_invalid_recv_window(self, monkeypatch):
        ...
        with pytest.raises(EnvironmentConfigurationError, match="PHOENIX_BYBIT_RECV_WINDOW_MS"):
            query_bybit_demo_open_orders(environ=_env(PHOENIX_BYBIT_RECV_WINDOW_MS="abc"))
        assert called == []
    ...
    def test_invalid_timeout(self, monkeypatch):
        ...
        with pytest.raises(EnvironmentConfigurationError, match="PHOENIX_HTTP_TIMEOUT_SECONDS"):
            query_bybit_demo_open_orders(environ=_env(PHOENIX_HTTP_TIMEOUT_SECONDS="abc"))
        assert called == []
...
class TestFailuresTranslatedToInfrastructureError:
    def test_socket_timeout_translated(self, monkeypatch):
        ...
        with pytest.raises(ExecutionInfrastructureError):
            query_bybit_demo_open_orders(environ=_VALID_ENV)
    ...
    def test_malformed_order_item_translated(self, monkeypatch):
        _install_fake_urlopen(monkeypatch, body=_ok_body(items=[_order_item(side="Unknown")]))
        with pytest.raises(ExecutionInfrastructureError):
            query_bybit_demo_open_orders(environ=_VALID_ENV)
    ...
    def test_bybit_processing_error_never_crosses_public_function(self, monkeypatch):
        _install_fake_urlopen(monkeypatch, body=b"{not json")
        error = _raised(lambda: query_bybit_demo_open_orders(environ=_VALID_ENV))
...
class TestEndpointAndMethod:
    def test_url_is_open_orders_endpoint(self, monkeypatch):
        calls = []
        _install_fake_urlopen(monkeypatch, capture=calls)
        query_bybit_demo_open_orders(environ=_VALID_ENV)
        assert calls[0]["url"].startswith("https://api-demo.bybit.com/v5/order/realtime?")
...
class TestRealTransportIntegration:
    ...
    def test_recv_window_header_matches_config(self, monkeypatch):
        calls = []
        _install_fake_urlopen(monkeypatch, capture=calls)
        query_bybit_demo_open_orders(environ=_env(PHOENIX_BYBIT_RECV_WINDOW_MS="7500"))
        headers_lower = {k.lower(): v for k, v in calls[0]["headers"].items()}
    ...
    def test_timeout_passed_to_urlopen_matches_config(self, monkeypatch):
        calls = []
        _install_fake_urlopen(monkeypatch, capture=calls)
        query_bybit_demo_open_orders(environ=_env(PHOENIX_HTTP_TIMEOUT_SECONDS="12.5"))
...
class TestSecurity:
    _MARKER = "ZZSUPERSECRETOPENORDERS9999"

    def test_marker_absent_from_result_repr(self, monkeypatch):
        _install_fake_urlopen(monkeypatch)
        result = query_bybit_demo_open_orders(
    ...
    def test_marker_absent_from_missing_variable_error(self):
        ...
        error = _raised(lambda: query_bybit_demo_open_orders(environ=env))
    ...
    def test_marker_absent_from_infrastructure_error_message(self, monkeypatch):
        ...
        error = _raised(lambda: query_bybit_demo_open_orders(
            environ=_env(PHOENIX_BYBIT_DEMO_API_SECRET=self._MARKER)
        ))
        assert error is not None
        assert self._MARKER not in str(error)
...
class TestTwoCallsIndependence:
    def test_two_calls_produce_distinct_results(self, monkeypatch):
        _install_fake_urlopen(monkeypatch, body=_ok_body(items=[_order_item(orderId="A")]))
        r1 = query_bybit_demo_open_orders(environ=_VALID_ENV)
        _install_fake_urlopen(monkeypatch, body=_ok_body(items=[_order_item(orderId="B")]))
        r2 = query_bybit_demo_open_orders(environ=_VALID_ENV)
        assert r1 is not r2
        assert r1.orders[0].exchange_order_id == "A"
        assert r2.orders[0].exchange_order_id == "B"
