...
- Ports/Adapters: a Port is a `Protocol` (`@runtime_checkable`); an Adapter implements it. `ExecutionGateway` (write) and the five read-side Ports (`PositionsReader`, `OpenOrdersReader`, `WalletBalanceReader`, `InstrumentMetadataReader`, `ExchangeStateReader`) are the only Ports that exist. See `docs/architecture.md` §5–6.
...
- **Bybit Demo is the only supported environment** (`D-011`). Do not assume Mainnet or Testnet exist as options; they are explicitly rejected.
